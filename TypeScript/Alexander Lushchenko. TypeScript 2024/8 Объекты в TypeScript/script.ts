...
const user2: { name: string; age: number | string; hasCar?: boolean } = {
  name: 'Alex',
  age: 20,
  // hasCar: false,
}
...
function userProcess(some_var: { name: string; age: number | string }): void {
...
// Декомпозиция
function foo({ name, age }: { name: string; age: number | string }) {
